...
(define machine
  (make-machine
    ...
    eval-dispatch
        ...
        (test (op let?) (reg exp))
        (branch (label ev-let))
        (goto (label unkown-expressiontype))
    ev-self-eval
        (assign val (reg exp))
        (goto (reg continue))
    ev-variable
        ...
        (goto (reg continue))
    ev-quoted
        (assign val (op text-op-quotation) (reg exp))
        (goto (reg continue))
    ev-lambda
    ...
    ev-application
        (save continue)
        (save anv)
        (assign uenv (op operands) (reg exp))
        (save uenv)
    ...
    ev-appl-did-operator
        ...
        (assign proc (reg val))
        (test (ope no-operands?) (reg uenv))
        (branch (label apply-dispatch))
        (save proc)
    ev-appl-operand-loop
        (save argl)
        (assign exp (op first-oprand) (reg uenv))
    ...
    ev-appl-accumulate-arg
        ...
        (assign uenv (op rest-oprands) (reg uenv))
        (goto (label ev-appl-operand-loop))
    ev-appl-last-arg
        (assign continue (label ev-appl-accumu-last-arg))
        (goto (label eval-dispatch))
    ev-appl-accumu-last-arg
        (restore argl)
        (assign argl (op adjoin-arg) (reg val) (reg argl))
        (restore proc)
    ...
    primitive-apply
        ...
        (goto (reg continue))
    compound-apply
        (assign uenv (op procedure-parameters) (reg proc))
        (assign env (op procedure-environment) (reg proc))
        (assign env (op extend-environment) (reg uenv) (reg argl) (reg env))
        (assign uenv (op procedure-body) (reg proc))
        (goto (label ev-sequence))
    ev-begin
        (assign uenv (op begin-actions) reg exp))
        (save continue)
        (goto (label ev-sequence))
    ev-sequence
        (asssign exp (op first-exp) (reg uenv))
        (test (op last-exp?) (reg uenv))
    ...
    ev-sequence-continue
        (restore env)
        (restore uenv)
        (asssign uenv (op rest-exps) (reg uenv))
        (goto (label ev-sequence))
    ...
    ev-assignment
        (assign uenv (op assignment-variable) (reg exp))
        (save uenv)
    ...
    ev-assignment-1
        ...
        (restore env)
        (restore uenv)
        (perform (op set-variable-value!) (reg uenv) (reg val) (reg env))
        (assign val (const ok))
        (goto (reg continue))
    ev-definition
        (assign uenv (op definition-variable) (reg exp))
        (save uenv)
    ...
    ev-definition-1
        ...
        (restore env)
        (restore uenv)
        (perform (op definition-variable!) (reg uenv) (reg val) (reg env))
        (assign val (const ok))
        (goto (reg continue))
    )))
